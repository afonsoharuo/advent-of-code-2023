...
func (c *Contraption) reset() {
	...
	for i := 0; i < m; i++ {
		for j := 0; j < n; j++ {
			(*c).Grid[i][j].Energised = false
			(*c).Grid[i][j].BeamDirections = make(map[Direction]bool)
		}
	}
}
...
func main() {
	...
	m := len(contraption.Grid)
	n := len(contraption.Grid[0])

	var numEnergised int
	maxNumEnergised := -1

	for i := 0; i < m; i++ {
		contraption.propagateBeam(i, 0, East)
		numEnergised = contraption.countEnergised()
		if numEnergised > maxNumEnergised {
			maxNumEnergised = numEnergised
		}
		contraption.reset()

		contraption.propagateBeam(i, m-1, West)
		numEnergised = contraption.countEnergised()
		if numEnergised > maxNumEnergised {
			maxNumEnergised = numEnergised
		}
	...
	for j := 0; j < n; j++ {
		contraption.propagateBeam(0, j, South)
		numEnergised = contraption.countEnergised()
		if numEnergised > maxNumEnergised {
			maxNumEnergised = numEnergised
		}
		...
		contraption.propagateBeam(0, n-1, North)
		numEnergised = contraption.countEnergised()
		if numEnergised > maxNumEnergised {
			maxNumEnergised = numEnergised
		}
